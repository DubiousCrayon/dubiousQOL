...
internal static class SourceIconResolver
{
    ...
    private static Texture2D? _otherIcon;
    ...
    public static ResolvedSource Resolve(string sourceName)
    {
        ...
        var result = TrySynthetic(sourceName)
                  ?? TryOrb(sourceName)
                  ...
                  ?? new ResolvedSource { Icon = LoadOtherIcon(), Kind = SourceKind.Other, BarColor = ColorOther, IconScale = 1f };
    ...
    public static void ClearCache()
    {
        ...
        _otherIcon = _moreIcon = null;
    }

    // Synthetic labels generated by the stats tracker, not real game entities.
    private static ResolvedSource? TrySynthetic(string name)
    {
        if (string.Equals(name, "Other", StringComparison.Ordinal))
            return new ResolvedSource { Icon = LoadOtherIcon(), Kind = SourceKind.Other, BarColor = ColorOther, IconScale = 1f };
        if (name.StartsWith("...", StringComparison.Ordinal))
            return new ResolvedSource { Icon = LoadMoreIcon(), Kind = SourceKind.Other, BarColor = ColorOther, IconScale = PaddedIconScale };
        return null;
    }
    ...
    {
        // First pass: match by localized title (exact).
        // Second pass: match by ID entry (case-insensitive). Some monsters have
        // different runtime titles than their ModelDb localized names (e.g.
        // Doormaker's ModelDb title is "Door" but combat records "Doormaker").
        MonsterModel? idMatch = null;
        foreach (var monster in ModelDb.Monsters)
        {
            ...
            if (string.Equals(title, name, StringComparison.Ordinal))
                return MakeMonsterSource(monster);

            if (idMatch == null && string.Equals(monster.Id.Entry, name, StringComparison.OrdinalIgnoreCase))
                idMatch = monster;
        }
        return idMatch != null ? MakeMonsterSource(idMatch) : null;
    }

    private static ResolvedSource MakeMonsterSource(MonsterModel monster)
    {
        Texture2D? icon;
        float scale;
        if (IsMinion(monster))
    ...
    private static Texture2D? LoadOtherIcon() =>
